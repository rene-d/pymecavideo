...
class film:
    ...
    def autoTest(self):
        ...
        try:
            ...
            self.data = cv.GetCaptureProperty(
                self.capture, cv.CV_CAP_PROP_FOURCC)
            print("CV_CAP_PROP_FOURCC : ", self.data)
            self.data = cv.GetCaptureProperty(
                self.capture, cv.CV_CAP_PROP_FRAME_COUNT)
