...
from ipl2qimage import * #tool opencv->qimage
from detect import filter_picture
import Error
#import Error
...
#                pass # il faut accepter que le combo soit vide à l'initialisation
    ...
    def affiche_image(self):
        ...
        print "affiche_image", self.filename, self.index_de_l_image

        print "affiche_image", self.filename, self.index_de_l_image
        cv.SetCaptureProperty(self.capture, cv.CV_CAP_PROP_POS_FRAMES,int(self.index_de_l_image))
        cv.SetCaptureProperty(self.capture, cv.CV_CAP_PROP_POS_FRAMES,int(self.index_de_l_image))
        cv.SetCaptureProperty(self.capture, cv.CV_CAP_PROP_POS_MSEC,float(int(self.index_de_l_image)*self.deltaT)*1000)
        frame = cv.QueryFrame(self.capture)
        image = QImage(IPLtoQPixmap(frame))
        print  "##############", self.index_de_l_image, cv.GetCaptureProperty(self.capture, cv.CV_CAP_PROP_POS_FRAMES), cv.GetCaptureProperty(self.capture, cv.CV_CAP_PROP_POS_MSEC)
        print "##############" , float(int(self.index_de_l_image)*self.deltaT)*1000
...
#            pass
    ...
    def openTheFile(self,filename):
        ...
        if filename != "" : 
            ...
            cv.SetCaptureProperty(self.capture, cv.CV_CAP_PROP_POS_FRAMES,1)
...
##            print args
            ...
            ##print "######### returncode", returncode
                ...
                #mauvaisevideo = QMessageBox.warning(self,self.tr(unicode("ERREUR","utf8")),
                #QString(self.tr(unicode("La video que vous tentez d'ouvrir n'est pas dans un format lisible.\n Merci d'en ouvrir une autre ou de l'encoder autrement","utf8"))),
#QMessageBox.Ok,QMessageBox.Ok)
    ...
    def recupere_avi_infos(self, fileName):
        ...
        framerate = cv.GetCaptureProperty(self.capture,cv.CV_CAP_PROP_FPS)
        nb_image = str(cv.GetCaptureProperty(self.capture,cv.CV_CAP_PROP_FRAME_COUNT))
        ...
        if pb :
            ...
            #print framerate, nb_image, p
        ######################################################################


        return framerate,int(float(nb_image))
