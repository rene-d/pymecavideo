...
class Label_Trajectoire(QLabel):
                ...
                for i in range(len(points)):
                    ...
                    try : 
                        if self.referentiel != 0:
                            ...
                            try:
                                ptreferentielAfter = self.label_video.app.points[key + 1][int(self.referentiel)]
                                ptreferentielBefore = self.label_video.app.points[key - 1][int(self.referentiel)]
                    ...
                    if type(point) != type(""):
                        if self.label_video.app.ui.radioButtonNearMouse.isChecked() and self.pos_souris != None:
                            ...
                            if distance.manhattanLength() < near:
                                self.label_video.app.dbg.p(2, "mouse near a point")
                                wroteSpeed = True
                        ...
                        if wroteSpeed:
                            ...
                            if key != 0 and key != keyMax - 1:  ##first and last point can't have speed.
                                ...
                                try:
                                    tempsAfter = float(self.label_video.app.points[key + 1][0])/self.label_video.app.deltaT  #en "delta_T"
                                    tempsBefore = float(self.label_video.app.points[key-1][0])/self.label_video.app.deltaT #en "delta_T"
                                    pointBefore = QPoint(
    ...
    def paintEvent(self, event):
       ...
       # self.painter.save()
        self.painter.fillRect(QRect(0, 0, self.label_video.width(), self.label_video.height()), QColor("grey"))
            ...
            for vector in self.speedToDraw:
                ...
                if vector_speed != QPoint():  #if speed is not null.
                    ...
                    try :
                        ...
                        self.label_video.app.ui.checkBoxScale.setStyleSheet("background-color:none");
                        path = QPainterPath()
                        path.moveTo(0, 0)
                        path.lineTo(speed, 0)
                        ...
                        self.label_video.app.checkBoxScale.setStyleSheet("background-color: red");
                    self.painter.end()
                else:
                    pass  #null speed
