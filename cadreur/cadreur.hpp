...
class Cadreur {
protected:
  const char *filename, * windowname, *fifoname;
  IplImage * img;
  CvCapture * film;
  bool end;
  ifstream * fifo;
public:
  Cadreur(const char *afilename = 0, const char * afifoname=0);
  ~Cadreur();
  void Run();
  void Loop(int max=-1);
  virtual void Restart();
  virtual bool queryFrame();     // queries a frame
  virtual void showImage();      // shows a clipped image
  void watchFifo();              // inputs signals from the fifo
  float fps();
  int count();
  int w();
  int h();
